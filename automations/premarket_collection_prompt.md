# v1.3.6 近期群聊与 Substack 通道修正（2026-09-19）

- 用户已现场确认抖音群聊网页长期只能回看近期消息且无法继续上划。每轮仍保存真实窗口、返回最新端并导入已核实的新消息；到达页面最早边界且滚动无进展后停止，不盲目循环。保持 `partial_failure` / `coverage_complete=false` 和完整游标冻结，不清除或伪造历史连续性，但不得把这一已知平台限制描述成语音转写、登录或新消息采集失败。
- 最终库当前完整群聊游标为 `2026-09-18 12:06 +08`，两小时重叠目标为 `10:06 +08`。页面最早可见时间是每轮现场事实，不在任务提示中固化为永久值；报告同轮实际边界、已入库/重复、语音/卡片待办和未核实区间。
- Substack 归档 API 必须通过已加载的持久化浏览器页面执行同源 `fetch()`，复用页面实际可用的登录和网络路径；不得退回独立 `page.request` 通道。页面请求只做有限重试，失败不推进最终库游标，其他来源继续。已授权会员正文仍按正常内容处理。
- 计划任务、仓库规则和每日入口以本节及后续最新规则为准。实际计划仍为北京时间 00:00、08:00、12:30、16:00、20:00，使用本地项目，不改变模型、推理强度或安全设置。

# 播放页与群聊加载修复（2026-09-18）

- 群聊入口必须等待动态渲染后的裸群名或带人数标题“宇菠萝的认知圈1群(人数)”，不得在页面 shell 初现时用一次性 `count()` 判定缺失。失败轮保存页面 URL、标题和截图；前两次复用授权 profile，第三次新建同 profile 标签重试。
- 最终库已核实覆盖原先误报的 2026-09-16 10:12 至 2026-09-17 21:01 区间，并有已审核群聊记录至 2026-09-18 12:06 +08。游标已通过最终库、重试队列和审计校正至 12:06；下一轮从 10:06 两小时重叠开始。网页只能回看近期消息属于长期 UI 回看限制；数据库既有历史已校正，但最终游标至同轮页面最早可见消息之间仍作为未核实连续区间保留。

- 公开作品 `blob:` 仅表示播放器使用临时地址。允许用现有公开抖音专用 profile 执行 `scripts/capture_douyin_playback.py --run-id <ID> --ids <本轮额度内ID...>`，读取正常播放的媒体响应、核实页面作者/权限/时长并下载真实响应体。脚本从最终库执行每来源最多3条、6小时冷却、5次上限；不使用强制重试。已有同轮失败尝试处于冷却时保留，不能换ID规避额度。
- 成功媒体仍须 `transcribe_douyin_batch.py` → `build_douyin_playback_recovery.py` → Codex全文复核 → `finalize_douyin_playback_review.py` → 标准事务导入。音频实际时长必须与播放页一致；ASR超出媒体尾端时必须复核和审计，不能直接标成功。
- 群聊打开后等待历史DOM稳定再写首个断点，防止临时作者/时间头消失引发误判；严格索引重叠校验保持。旧队列因相对时间或初始批次边界变化留下别名时，可运行 `reconcile_group_retry_evidence.py --source-run <原轮> --run-id <补齐轮>`；仅接受经哈希验证的连续窗口、唯一带日期的非语音文字锚点以及最终库已提交正文，保留原证据和销账审计。
...
- 用户确认久韭究财 `7686153728089015993` 是订阅重复项，永久排除下载、转录重试和入库；保留订阅记录 `7686152973647700197`。旧待审文件不得重新导入错误项。
- 没有采到会员标签只能记作权限未确认，不能据此断言公开或无音轨。下载或播放页补全前应核实对应作品的实际订阅标签/权限；出现订阅证据立即停止公开补全并按 review_only 处理。不同 ID 同标题只作为复查线索，不自动合并其他作品。
- 本轮仍有未补齐时分别报告尝试与最终库实际更新数；不得把重试记录当作补全成功。
